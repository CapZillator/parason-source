...
export function MainChart(){
  ...
  if (weather){
    ...
    circleTMinPoints = pointsTMin.map((p, i) => {
      return <circle key={`tMinCircle${i}`} cx={p.x} cy={p.y} r={circleTradius} style={{fill: 'black', stroke: 'none'}}/>;
    });
  ...
  return (
        <div className="Chart-wrapper">
          <svg width="1000" height="750" viewBox="0 0 1000 750" className="Chart-component">
            ...
            <rect x="0" y="0" width='1000' height='750' style={{fill: 'rgba(255, 255, 255, .05)'}}/>
            {pressureColumns}
            {precipColumns}
            <path d={pathTMax}
              className="pathTMax" style={{fill: 'url(#TGradient)'}}/>
            <path d={pathTMin} strokeWidth="4" className="PathTMin" />
